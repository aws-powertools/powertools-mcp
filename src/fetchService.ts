...
class FetchService {
   ...
   */
  // biome-ignore lint/suspicious/noExplicitAny: TODO see if we can narrow this down
  ...
  constructor(config: CacheConfig) {
    ...
    if (config.contentTypes) {
      for (const [type, settings] of Object.entries(config.contentTypes)) {
  ...
  private determineContentType(
    ...
    if (urlString.includes('markdown.local')) {
      return ContentTypeMap.MARKDOWN;
    }
    return ContentTypeMap.WEB_PAGE;
  }
  ...
  public fetchWithContentType(
    ...
    if (!fetchInstance) {
      throw new Error(
        `No fetch instance configured for content type: ${contentType}`
      );
    }
