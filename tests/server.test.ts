...
vi.mock('../src/searchIndex.ts', async (importOriginal) => {
  return {
    SearchIndexFactory: vi.fn().mockImplementation(() => ({
...
it('returns an error with available versions when an invalid version is provided', async () => {
  // Prepare
  mocks.resolveVersion.mockResolvedValueOnce({
  ...
  const handleRequest = async (request: any) => {
    try {
      ...
      if (name === 'search_docs') {
        ...
        const runtime = args.runtime.trim().toLowerCase();
        const version = args.version?.trim().toLowerCase() || 'latest';

        // Check if the version is valid
        const versionInfo = await mockResolveVersion(runtime, version);
        if (!versionInfo.valid) {
          // Return an error with available versions
          const availableVersions =
            versionInfo.available?.map((v: any) => ({
              version: v.version,
              title: v.title,
              aliases: v.aliases,
            })) || [];
...
it('returns empty available versions array when no versions are found', async () => {
